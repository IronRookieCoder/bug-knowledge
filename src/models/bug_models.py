...
class BugReport(BaseModel):
    ...
    file_paths: List[str] = Field(default_factory=list, description="受影响的文件路径列表")
    code_diffs: List[str] = Field(default_factory=list, description="代码差异列表")
    aggregated_added_code: Optional[str] = Field("", description="所有新增代码的聚合")
    aggregated_removed_code: Optional[str] = Field("", description="所有删除代码的聚合")
    test_steps: Optional[str] = Field("", description="重现缺陷的测试步骤")
    expected_result: Optional[str] = Field("", description="预期结果")
    actual_result: Optional[str] = Field("", description="实际结果")
    log_info: Optional[str] = Field("", description="相关日志信息")
    severity: Optional[str] = Field("P3", description="缺陷的严重程度")
    is_reappear: Optional[str] = Field("未知", description="缺陷是否可重现")
    environment: Optional[str] = Field("未知", description="缺陷发生的环境信息")
    ...
    handlers: List[str] = Field(default_factory=list, description="处理该缺陷的人员列表")
    project_id: Optional[str] = Field("default", description="关联gitlab仓库id")
