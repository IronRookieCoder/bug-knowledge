...
class CodeFeatureExtractor:
            ...
            def visit_Try(self, node):
                ...
                for handler in node.handlers:
                    ...
                    features["exception_handlers"].append(
                        {"type": handler_type, "body_length": len(handler.body)}
                    )
    ...
    def calculate_similarity(
        self, query_features: CodeFeatures, target_features: CodeFeatures
    ) -> float:
        """计算代码特征相似度"""
        scores = {
            ...
            "symbolic": self._calculate_symbolic_similarity(
                query_features, target_features
            ),
        }

        return sum(
            score * self.feature_weights[weight_type]
            for weight_type, score in scores.items()
        )

    def _calculate_semantic_similarity(
        self, query: CodeFeatures, target: CodeFeatures
    ) -> float:
        """计算语义相似度"""
        ...
        func_sim = (
            len(query_funcs & target_funcs) / len(query_funcs | target_funcs)
            if query_funcs or target_funcs
            else 0
        )
        class_sim = (
            len(query_classes & target_classes) / len(query_classes | target_classes)
            if query_classes or target_classes
            else 0
        )
        api_sim = (
            len(query_apis & target_apis) / len(query_apis | target_apis)
            if query_apis or target_apis
            else 0
        )
    ...
    def _calculate_structural_similarity(
        ...
        depth_diff = abs(
            query.structure_features["nesting_depth"]
        ...
        control_sim = (
            sum(
                min(query_controls[k], target_controls[k])
                for k in set(query_controls) | set(target_controls)
            )
            / sum(
                max(query_controls[k], target_controls[k])
                for k in set(query_controls) | set(target_controls)
            )
            if query_controls or target_controls
            else 0
        )

        return (depth_sim + control_sim) / 2

    def _calculate_symbolic_similarity(
        self, query: CodeFeatures, target: CodeFeatures
    ) -> float:
        """计算符号相似度"""
        # 比较变量名、函数名和类名的模式相似度
        ...
        pattern_sim = (
            sum(
                min(query_patterns[k], target_patterns[k])
                for k in set(query_patterns) | set(target_patterns)
            )
            / sum(
                max(query_patterns[k], target_patterns[k])
                for k in set(query_patterns) | set(target_patterns)
            )
            if query_patterns or target_patterns
            else 0
        )
    ...
    def _calculate_symbol_score(
        self, query_symbols: List[str], target_symbols: List[str]
    ) -> float:
        """计算符号相似度分数
        ...
        for q_sym in query_symbols:
            ...
            for t_sym in target_symbols:
                # 计算LCS得分
                lcs_score = (
                    2 * lcs_length(q_sym, t_sym) / (len(q_sym) + len(t_sym))
                    if q_sym and t_sym
                    else 0
                )
                # 计算编辑距离得分
                edit_score = (
                    1 - edit_distance(q_sym, t_sym) / max(len(q_sym), len(t_sym))
                    if q_sym and t_sym
                    else 0
                )
                # 综合得分
    ...
    def _is_structure_similar(
        self, query_structure: Dict[str, Any], target_structure: Dict[str, Any]
    ) -> bool:
        """判断两个代码结构是否相似
        ...
        # 1. 检查控制结构数量是否接近
        def is_control_count_similar(
            q_count: Dict[str, int], t_count: Dict[str, int], threshold: float = 0.3
        ) -> bool:
        ...
        # 3. 检查异常处理是否相似
        def is_exception_similar(
            q_handlers: List[Dict], t_handlers: List[Dict], threshold: float = 0.5
        ) -> bool:
        ...
        depth_similar = is_depth_similar(
            query_structure["nesting_depth"], target_structure["nesting_depth"]
        )
