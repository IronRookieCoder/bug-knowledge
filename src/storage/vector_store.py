...
class VectorStore:
    ...
    def _build_and_save_index(
        self, index: Optional[AnnoyIndex], name: str
    ) -> Optional[AnnoyIndex]:
        """构建并保存索引"""
        if not index:
        ...
        try:
            ...
            logger.debug(
                f"尝试卸载传入的索引对象 '{name}' (ID: {id(index)}) 以释放潜在锁..."
            )
            try:
                index.unload()
                ...
                logger.warning(
                    f"卸载传入的索引对象 '{name}' 时发生错误 (可能无影响): {e_unload1}"
                )

            # 额外保障：获取并尝试卸载当前 self.<name>_index 属性持有的实例
            current_self_index = getattr(self, f"{name}_index", None)
            if (
                current_self_index and current_self_index is not index
            ):  # 避免重复卸载同一个对象
                logger.debug(
                    f"尝试卸载存储在 self.{name}_index 的旧实例 (ID: {id(current_self_index)}) ..."
                )
                try:
                    ...
                    logger.warning(
                        f"卸载旧的 self.{name}_index 实例时发生错误: {e_unload2}"
                    )
            elif current_self_index and current_self_index is index:
                logger.debug(
                    f"存储在 self.{name}_index 的实例与传入实例相同，无需重复卸载。"
                )
            ...
            logger.info(
                f"索引 '{name}' 重新加载成功，包含 {new_index.get_n_items()} 个项目。"
            )
            ...
            logger.error(
                f"处理（构建、保存、替换或重新加载）索引 '{name}' 过程中失败: {str(e)}"
            )
            logger.error(f"错误堆栈: {traceback.format_exc()}")
            # 清理临时文件
            if temp_path.exists():
    ...
    def _cached_search(
        self, vector_key: str, vector: Tuple[float], n_results: int
    ) -> Dict[int, float]:
        """缓存搜索结果"""
        ...
        try:
            ...
            ids, distances = index.get_nns_by_vector(
                list(vector), n_results, include_distances=True, search_k=100
            )
    ...
    def add_bug_report(self, bug_report: Any, vectors: Dict[str, Any]):
        ...
        try:
            ...
            # 合并测试相关字段的向量
            if (
                "test_steps_vector" in vectors
                or "expected_result_vector" in vectors
                or "actual_result_vector" in vectors
            ):
                test_info = {
            ...
            for index_name, vector_key in index_definitions:
                    ...
                    try:
                        ...
                        for item_id in range(old_index.get_n_items()):
                            try:
                                ...
                                logger.warning(
                                    f"Index {index_name}: 复制向量 ID {item_id} 失败: {e}"
                                )
                        old_index.unload()
                    except Exception as e:
                ...
                if vector_key in vectors and vectors[vector_key] is not None:
                    try:
                        ...
                        logger.error(
                            f"Index {index_name}: 添加新项目 ID {db_id} 时出错: {e_add_new}"
                        )
                        success = False
                        break
                else:
                    logger.debug(
                        f"Index {index_name}: 没有为新项目提供向量 (key: {vector_key})。"
                    )
    ...
    def search(
        ...
        try:
            ...
            similarity_distances = [
                result.get("distance", float("inf")) for result in vector_results
            ]
    ...
    def _load_all_indices(self, for_read: bool = True):
        ...
        for name in index_names:
            try:
                ...
                if loaded_index:
                    logger.info(
                        f"成功加载索引 {name}，包含 {loaded_index.get_n_items()} 个项目"
                    )
                else:
